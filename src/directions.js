 ...
 */
const getCardinalPoints = shortCode => {
  switch (shortCode) {
    case 'N':
      return 'NORTH';
    case 'S':
...
const isValidCoordinate = coordinate => {
  ...
  if ((!testX || !testY) && process.env.NODE_ENV !== 'test') {
    communication.error(`You cannot place at the coordinate of [${coordinate[0]},${coordinate[1]}] as it will make your robot falls off, please change direction or key in another coordinate`);
  }
  return testX && testY;
};
...
const isValidDirection = direction => {
  ...
  if (!result && process.env.NODE_ENV !== 'test') {
    communication.error(`Please key in one of the valid directions: N, S, E, W`);
  }
  return result;
};

module.exports = { getCardinalPoints, getNewDirection, getNewCoordinate, isValidCoordinate, isValidDirection };
